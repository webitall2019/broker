...
function cg_register_production_js() {
	...
	if ( !is_admin() ) {
		...
		wp_enqueue_script( 'cg_modernizr_custom', CG_JS . '/src/cond/modernizr.js', array( 'jquery' ), '', false );
...
//line 35 above-> add_action( 'init', 'cg_register_production_js' );
//add_action( 'wp_enqueue_scripts', 'cg_register_debug_js' );
// load projects scripts only on projects template
function cg_projects_js() {
	if ( is_page_template( 'template-projects-landing.php' ) )  {
		wp_enqueue_script( 'cg_isotope', CG_JS . '/src/cond/isotope.pkgd.min.js', array( 'jquery' ), '1.0', false );
	}
}

add_action( 'wp_enqueue_scripts', 'cg_projects_js' );
